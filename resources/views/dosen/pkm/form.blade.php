        ...
        <div class="card border-0 shadow-sm mb-3">
            ...
            <div class="card-body">
                <table class="table table-bordered align-middle">
                    ...
                    <tfoot><tr class="fw-bold"><td colspan="7" class="text-end">TOTAL:</td><td id="rabTotal" class="text-end">Rp 0</td><td></td></tr></tfoot>
                </table>
                <button type="button" class="btn btn-sm btn-outline-primary" onclick="addRab()"><i class="ri-add-line"></i> Tambah Baris RAB</button>
            </div>
        </div>

        {{-- Section 9b: Dokumen Pendukung (lampiran) --}}
        @if ($isEdit)
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-header bg-white">
                    <h6 class="mb-0"><i class="ri-attachment-2 text-primary me-2"></i>9b. Dokumen Pendukung (Lampiran)</h6>
                </div>
                <div class="card-body">
                    <p class="text-muted small mb-3">Wajib untuk PKM: <strong>Surat Kesediaan Mitra</strong>. Lainnya opsional: KTP Ketua, SK Mengajar, MoU, dll. PDF / JPG / PNG &middot; maks 5 MB.</p>
                        ...
                        <tbody>
                            @forelse ($dokumenList as $d)
                                <tr>
                                    ...
                                    <td>
                                        <form method="POST" action="{{ route('dosen.pkm.dokumen.delete', [$proposal, $d]) }}" class="d-inline" onsubmit="return confirm('Hapus dokumen ini?')">
                                            @csrf @method('DELETE')
                    ...
                    <form method="POST" action="{{ route('dosen.pkm.dokumen.upload', $proposal) }}" enctype="multipart/form-data" class="row g-2 align-items-end">
                        @csrf
                        <div class="col-md-4">
                            <label class="form-label small">Jenis Dokumen</label>
                            <select name="jenis" class="form-select form-select-sm" required>
                                <option value="">-- Pilih --</option>
                                <option value="Surat Kesediaan Mitra">Surat Kesediaan Mitra</option>
                                ...
                                <option value="MoU">MoU / Kerjasama Mitra</option>
...
function filterKomponen(kategoriSelect) {
    ...
    komponenSelect.querySelectorAll('option').forEach(opt => {
        ...
        opt.hidden = opt.dataset.kategori !== kategoriId;
    });
