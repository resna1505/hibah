...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted small mb-0">Buat akun login (NIK + password). Data pribadi lengkap diisi sendiri oleh dosen setelah login.</p>
        </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <form method="GET" class="row g-2">
                <div class="col-md-4">
                    <input type="text" name="q" value="{{ $q }}" class="form-control form-control-sm" placeholder="Cari NIK / nama / email...">
                </div>
                <div class="col-auto">
                        ...
                        <div class="modal fade" id="mReset{{ $u->id }}" tabindex="-1">
                            <div class="modal-dialog">
                                <form method="POST" action="{{ route('operator.akun-dosen.reset-password', $u) }}" class="modal-content">
                                    @csrf @method('PATCH')
                                    <div class="modal-header"><h6 class="modal-title">Reset Password — {{ $u->nik }}</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    <div class="modal fade" id="mTambah" tabindex="-1">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('operator.akun-dosen.store') }}" class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-2"><label class="form-label">NIK <span class="text-danger">*</span></label><input type="text" name="nik" class="form-control" required value="{{ old('nik') }}"><small class="text-muted">Dipakai untuk login. Username otomatis = NIK.</small></div>
